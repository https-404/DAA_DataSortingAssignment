...
def merge_sort_descending(arr, column_name):
    return merge_sort(arr, column_name, descending=True)

# Merge sort implementation
def merge_sort(arr, column_name, descending=False):
    if len(arr) > 1:
        mid = len(arr) // 2  # Corrected this line
        left_half = arr[:mid]
        right_half = arr[mid:]
...
# Apply the merge sort algorithm to sort the data based on the "Number of Credit Problems" column in descending order
merge_sort_descending(data_list, "Number of Credit Problems")
...
sorted_data.to_excel("SortedDataset_MergeSort_Descending_NumberOfCreditProblems.xlsx", index=False)
