...
def merge_sort(arr, column_name, descending=True):
    if len(arr) > 1:
        mid = len(arr) // 2
        left_half = arr[:mid]
        right_half = arr[mid:]
...
# Apply the merge sort algorithm to sort the data based on the "Years of Credit History" column in descending order
merge_sort(data_list, "Years of Credit History", descending=True)
...
sorted_data.to_excel("SortedDataset_MergeSort_Descending_YearsOfCreditHistory.xlsx", index=False)
