...
def merge_sort(arr, descending=False):
    if len(arr) > 1:
        ...
        while i < len(left_half) and j < len(right_half):
            if (left_half[i] > right_half[j]) if descending else (left_half[i] < right_half[j]):
                arr[k] = left_half[i]
                i += 1
            else:
                arr[k] = right_half[j]
...
# Apply the merge sort algorithm to the "Annual Income" column in descending order
merge_sort(annual_income_column, descending=True)
...
sorted_data = data
sorted_data.to_excel("SortedDataset_MergeSort_Descending.xlsx", index=False)
