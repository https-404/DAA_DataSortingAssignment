...
def merge_sort_descending(arr, column_name):
    if len(arr) > 1:
        ...
        while i < len(left_half) and j < len(right_half):
            ...
            # Handle "NA" values by replacing with the median
            if left_value == "NA":
                left_value = median
            if right_value == "NA":
                right_value = median
...
# Apply the modified merge sort algorithm to sort the data based on the "Months since last delinquent" column in descending order
merge_sort_descending(data_list, "Months since last delinquent")
...
sorted_data.to_excel("SortedDataset_MergeSort_Descending_MonthsSinceLastDelinquent.xlsx", index=False)
